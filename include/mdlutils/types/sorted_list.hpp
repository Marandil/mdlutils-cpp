...
namespace mdl
{
    ...
    public:
        ...
        typedef typename base_type::const_iterator iterator; // const iterator to prevent modifying already inserted values
        typedef typename base_type::const_iterator const_iterator;
        typedef typename base_type::const_reverse_iterator reverse_iterator; // const iterator to prevent modifying already inserted values
    ...
    public:
        ...
        iterator emplace_hint(iterator position, Args&&... args)
        {
            ...
            if(comp(object, *it)) // the value is smaller than the hinted position, therefore insert it to the left:
            {
                ...
                return base.insert(++it, std::move(object)); // we need to insert at an element greater than the smaller one
            }
        ...
        size_t erase(iterator position, const T& value)
        {
            ...
            size_t removed = 0;
            while(equiv(position, value)) // '!( < || > )' <==> '==', shortcut if hinted == value
            {
    ...
    bool operator== ( const sorted_list<T,Compare,Allocator>& lhs,
    ...
    bool operator== ( const std::multiset<T,Compare,Allocator>& lhs,
                      const sorted_list<T,Compare,Allocator>& rhs )
    {
        return rhs == lhs;
    };
